        ...
        <div class="col-sm-12">
            ...
            <table class="table">
                ...
                @foreach($user->invoices as $invoice)
                    <tr><td>{{$invoice->title}}</td><td>{{$invoice->amount()}}</td><td>@if($payment->status===1) <span class="green-text">Paid</span> @else <span class="yellow-text">Pending</span> @endif</td></tr>
                @endforeach
            </table>
        </div>
    </div>
